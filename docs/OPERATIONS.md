...
`check:auth`, `check:workflow` ve `check:pwa` çalışan Compose stack'i bekler. `check:workflow`; müşteri işlemi, Admin salt-okunur kontrolü, Supervisor ataması/risk override, Gamification SSE stream, Analyst kararı, RabbitMQ puanı, müşteri yıldız feedback'i ve leaderboard zincirini gerçek BFF üzerinden doğrular.
