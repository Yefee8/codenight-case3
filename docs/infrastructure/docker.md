...
| RabbitMQ | messaging | ayrı volume | yok |
...
- Uygulama kendi DB readiness'ini ve tek seferlik RabbitMQ topology/ACL bootstrap'ını bekler.
- AI ve Game cache gibi degrade edilebilir dependency'nin health'i domain servisini boot
  aşamasında kilitlemez; client timeout/fallback çalışır.
...
- Java JRE, Python virtualenv wheels, Next.js standalone Node katmanı.
