...
GitHub Actions zaten ekli olduğu için en küçük tam bonus olarak **SSE canlı puan bildirimi (+2)** seçildi ve eklendi.

| Aday madde | Durum | Karar |
|---|---|---|
| 9.1 AI veri/model | Mevcut AI deterministik kural motoru; gerçek dataset/train/model registry tam iş değil | Büyük kapsam, bu turda seçilmedi |
...
| AI | FastAPI, SQLAlchemy, PostgreSQL | Risk skoru, fraud tipi, karar, prediction kaydı | `ai-db` |
...
| 9.1 AI veri/model | Kural bazlı AI var; gerçek train edilmiş model/dataset registry yok | Kısmi |
...
| Bonus kategori doğruluğu | Dashboard AI availability var; gerçek category accuracy metriği yok | 0 |
| Bonus özel model | Deterministik model var; özel ML artifact yok | 0 |

Beklenen skor, mimari ve kod kalitesi hariç:

- Zorunlu kalemler: **yaklaşık 58-62 / 65**
- Bonus: **9 / 20** (`RabbitMQ +5`, `SSE +2`, `GitHub Actions +2`)
- En büyük puan riski: **9.1 gerçek AI model/dataset** ve **11.2 tam SLA scheduler**.

Sunumda güvenli ifade:

> "Zorunlu canlı demo, güvenlik testleri, feedback, RLS ve RabbitMQ akışı hazır. Bonus tarafında RabbitMQ, SSE ve GitHub Actions tamam. Gerçek ML artifact ve tam SLA scheduler üretimleşme kapsamı olarak bırakıldı."
...
Backend tarafında Transaction Service en kritik bounded context'tir. İşlem ve vaka aynı servis DB'sinde tutulur; müşteri kimliği request body'den değil token'dan alınır. AI cevap vermezse Transaction fail etmez, case'i manuel incelemeye alır. Böylece servis kapatma demosunda sistem ayakta kalır.
