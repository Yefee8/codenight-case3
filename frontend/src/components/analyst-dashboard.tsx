...
import { useApiErrorToast, useApproveTransaction, useGetCase, useGetCases, useGetGameProfile } from "@/hooks/use-fraudcell";
import { useFraudcellEvents } from "@/hooks/use-fraudcell-events";
import { dateTime, money } from "@/lib/utils";
import { fraudLabels } from "@/lib/domain-labels";
import type { AnalystDecision } from "@/types/domain";

/** Interactive analyst controls load role-filtered data from the Gateway. */
export function AnalystDashboard({ userId }: { userId: string }) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [note, setNote] = useState("");
  const cases = useGetCases();
  const profile = useGetGameProfile();
  const assigned = cases.data?.filter((item) => item.assigned_analyst_id === userId) ?? [];
  const activeId = selectedId ?? assigned[0]?.case_id ?? null;
  const detail = useGetCase(activeId);
  const decision = useApproveTransaction();
  useFraudcellEvents("all");
  useApiErrorToast(cases.error, "Vaka kuyruğu yüklenemedi");
  useApiErrorToast(detail.error, "Vaka detayı yüklenemedi");
  useApiErrorToast(profile.error, "Analist profili yüklenemedi");

  async function decide(value: AnalystDecision) {
    if (!activeId || !detail.data || detail.data.version === undefined || !note.trim()) return;
    try {
      await decision.mutateAsync({ id: activeId, decision: value, note, version: detail.data.version, status: detail.data.status });
      setNote("");
  ...
  return (
    <>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3">
        ...
        <Summary icon={<Target />} label="Bugünkü doğruluk" value="—" />
      </div>
              ...
              <Table>
                ...
                <TableBody>{assigned.map((item) => (
                  <TableRow key={item.case_id} onClick={() => setSelectedId(item.case_id)} className={activeId === item.case_id ? "bg-brand-soft" : "cursor-pointer"}>
                    <TableCell><span className="flex items-center gap-2 font-mono text-xs"><i className={`size-1.5 rounded-full ${item.risk_level === "KRITIK" ? "bg-red-500 shadow-[0_0_8px_#ef4444]" : "bg-accent"}`} />{item.transaction_details.transaction_number ?? item.case_id}</span></TableCell>
        ...
        <Card>
          ...
          <CardContent>
            {profile.isLoading ? <Skeleton className="h-36" /> : profile.data && <>
              ...
              <div className="mb-5 flex items-center justify-between rounded-xl bg-muted p-3"><span className="text-xs text-muted-foreground">Günlük sıra</span><strong className="text-xl">{profile.data.daily_rank === null ? "—" : `#${profile.data.daily_rank}`}</strong></div>
...
function CaseDetail({ item, loading, note, setNote, pending, decide }: { item?: ReturnType<typeof useGetCase>["data"]; loading: boolean; note: string; setNote: (value: string) => void; pending?: AnalystDecision; decide: (value: AnalystDecision) => void }) {
  if (loading) return <Card><CardContent><Skeleton className="h-[460px]" /></CardContent></Card>;
  if (!item) return <Card><CardContent className="grid min-h-[460px] place-items-center text-center text-sm text-muted-foreground">İncelemek için bir vaka seçin.</CardContent></Card>;
  const canDecide = item.status === "ATANDI" || item.status === "INCELENIYOR";
  return <Card>
    <CardHeader><div className="flex items-center justify-between"><CardTitle>Vaka detayı</CardTitle><RiskBadge risk={item.risk_level} /></div><p className="font-mono text-xs text-muted-foreground">{item.transaction_details.transaction_number ?? item.case_id}</p></CardHeader>
    <CardContent className="space-y-5">
      <div className="flex items-center justify-between rounded-xl bg-muted p-4"><div><p className="text-xs text-muted-foreground">AI risk skoru</p><p className="text-3xl font-semibold">{item.ai_analysis.risk_score === null ? "BELİRSİZ" : `%${Math.round(item.ai_analysis.risk_score * 100)}`}</p></div><Sparkles className="text-accent" /><div className="text-right"><p className="text-xs text-muted-foreground">Öneri</p><strong>{item.ai_analysis.recommended_decision}</strong></div></div>
      ...
      <div className="grid grid-cols-2 gap-3"><Button variant="outline" loading={pending === "ONAYLANDI"} disabled={!canDecide || !note.trim() || Boolean(pending)} onClick={() => decide("ONAYLANDI")}>Onayla</Button><Button variant="danger" loading={pending === "BLOKLANDI"} disabled={!canDecide || !note.trim() || Boolean(pending)} onClick={() => decide("BLOKLANDI")}>Blokla</Button></div>
    </CardContent>
  </Card>;
}
