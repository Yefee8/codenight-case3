...
export function AnalystDashboard({ initialCases, initialProfile, userId }: { initialCases: TransactionCase[]; initialProfile: GamificationProfile; userId: string }) {
  ...
  async function start() {
    if (!activeId) return;
    try {
      await startReview.mutateAsync(activeId);
      toast.success("Vaka incelemeye alındı");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "İnceleme başlatılamadı");
    }
  }
  ...
  return (
    <>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3">
        ...
        <Summary icon={<Target />} label="İncelenen" value={assigned.filter((item) => item.status === "INCELENIYOR").length} />
      </div>
      <div className="grid items-stretch gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,.75fr)_280px]">
        ...
        <CaseDetail item={detail.data} loading={detail.isLoading} note={note} setNote={setNote} pending={decision.isPending ? decision.variables?.decision : undefined} starting={startReview.isPending} start={start} decide={decide} />
        ...
        <Card>
          ...
          <CardContent>
            {profile.isLoading ? <Skeleton className="h-36" /> : profile.data && <>
              ...
              <div className="mb-5 flex items-center justify-between rounded-xl bg-muted p-3"><span className="text-xs text-muted-foreground">Günlük sıra</span><strong className="text-xl">{profile.data.daily_rank ? `#${profile.data.daily_rank}` : "—"}</strong></div>
...
function CaseDetail({ item, loading, note, setNote, pending, starting, start, decide }: { item?: ReturnType<typeof useGetCase>["data"]; loading: boolean; note: string; setNote: (value: string) => void; pending?: AnalystDecision; starting: boolean; start: () => void; decide: (value: AnalystDecision) => void }) {
  if (loading) return <Card><CardContent><Skeleton className="h-[460px]" /></CardContent></Card>;
  if (!item) return <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">Size atanmış vaka bulunmuyor.</CardContent></Card>;
  const score = item.ai_analysis.risk_score;
  return <Card>
    ...
    <CardContent className="space-y-5">
      ...
      </> : <p className="rounded-xl bg-muted p-3 text-sm text-muted-foreground">Bu vaka için şu anda karar işlemi yapılamaz.</p>}
    </CardContent>
  </Card>;
}
