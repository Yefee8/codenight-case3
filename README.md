...
- React 19 + TypeScript: dört role ait basic test/demonstrasyon UI'ı.
...
| RabbitMQ UI | `http://127.0.0.1:15672` | Yalnız local operasyon |
| PostgreSQL/Redis/domain servisleri | host'a kapalı | Docker internal network |
...
Servis başına least-privilege broker hesabını açmak için `.env.rabbitmq-acl.example`
değerlerini secret olarak sağlayıp hardened overlay kullanılır:

```powershell
docker compose `
  -f docker-compose.yml `
  -f docker-compose.override.yml `
  -f docker-compose.rabbitmq-hardened.yml `
  up --build --wait
```
...
Set-Location frontend; npm ci; npm run test:coverage
```
